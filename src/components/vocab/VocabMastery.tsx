...
const UI = {
  ...
  // A `cards: recognition` word has one card, not two, so it can never show production
  // evidence. Saying so HERE matters more than in VocabTable.astro: an unowned completion
  // deck — exactly what recognition-only entries are for — is only ever reached through
  // /vocab/[id], which renders this component and not that table. (Codex review, PR #92.)
  recognitionOnly: { en: 'understand only', ru: 'только на понимание' },
  recognitionOnlyWhy: {
    en: 'One card only: this word is here to be understood, not produced.',
    ru: 'Только одна карточка: это слово нужно понимать, а не воспроизводить.',
  },
...
export function VocabWordTable({ cards }: { cards: CardDef[] }) {
  ...
  return <div>
    ...
    <div className="mt-4 divide-y divide-stone-200 rounded-lg border border-stone-200 dark:divide-stone-700 dark:border-stone-700">{visible.map((word) => { const def = defs.get(word.key)!; return <details key={word.key} className="group p-3"><summary className="flex cursor-pointer list-none items-center gap-3"><span className={`h-2.5 w-2.5 rounded-full ${COLORS[word.status]}`} /><span className="font-semibold">{word.de}</span><span className="text-sm text-stone-500">{pick(lang, { en: def.en, ru: def.ru, uk: def.uk })}</span>{word.recognitionOnly && <span className="rounded bg-stone-100 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-stone-500 dark:bg-stone-700 dark:text-stone-300">{pick(lang, UI.recognitionOnly)}</span>}{word.due && <span className="ml-auto rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800 dark:bg-amber-900 dark:text-amber-200">{t('vocab.dueChip', uiLang)}</span>}<span className={word.due ? '' : 'ml-auto'}>›</span></summary>{word.recognitionOnly && <p className="mt-2 pl-5 text-xs text-stone-400">{pick(lang, UI.recognitionOnlyWhy)}</p>}<div className="mt-3 grid gap-2 pl-5 text-xs text-stone-500 sm:grid-cols-2">{(word.recognitionOnly ? (['de-x'] as const) : (['de-x','x-de'] as const)).map((dir) => { const card = word.directions[dir].card; return <div key={dir} className="rounded bg-stone-50 p-2 dark:bg-stone-900/50"><b className="text-stone-700 dark:text-stone-300">{t(dir === 'de-x' ? 'vocab.dirRecognize' : 'vocab.dirProduce', uiLang)}</b><p className="mt-1">{card ? t('vocab.cardStats', uiLang).replace('{reps}', String(card.reps)).replace('{days}', String(card.scheduled_days)).replace('{lapses}', String(card.lapses)) : t('vocab.notStarted', uiLang)}</p>{card?.due && <p>{t('vocab.dueDate', uiLang).replace('{date}', new Date(card.due).toLocaleDateString())}</p>}</div>; })}</div></details>; })}{visible.length === 0 && <p className="p-4 text-sm text-stone-400">{t('vocab.emptyFilter', uiLang)}</p>}</div>
  </div>;
}
...
//
// Only the first group (the per-level bars) is open by default; the per-topic
// and per-POS breakdowns sit behind one <details>. Render-only: the groups prop
// and its card-id lists are unchanged, nothing new is computed or inlined.
export function VocabularyProgress({ cards, groups, states }: { cards: CardDef[]; groups: Array<{ title: StringKey; items: VocabGroup[] }>; states: CardStates }) {
  const lang = useExplainLang();
  const uiLang = useUiLang();
  const renderGroup = (group: { title: StringKey; items: VocabGroup[] }) => <div key={group.title} className="mt-5"><h3 className="text-xs font-semibold uppercase tracking-wide text-stone-400">{t(group.title, uiLang)}{group.title === 'filter.level' && <span className="ml-2 font-normal normal-case tracking-normal">· {pick(lang, UI.currentlyAuthored)}</span>}</h3><div className="mt-2 grid gap-3 sm:grid-cols-2">{group.items.map((item) => { const ids = new Set(item.cardIds); const words = rollupWords(cards.filter((card) => ids.has(card.id)), states); return words.length ? <div key={item.id} className="rounded-md bg-stone-50 p-3 dark:bg-stone-900/50"><div className="mb-2 flex justify-between gap-2 text-sm"><span className="font-medium">{item.label}</span><span className="text-stone-400">{words.length}</span></div><MasteryBar counts={masteryCounts(words)} compact /></div> : null; })}</div></div>;
  const [first, ...rest] = groups;
  return <section className="rounded-lg border border-stone-200 bg-white p-6 dark:border-stone-700 dark:bg-stone-800"><h2 className="text-sm font-semibold text-stone-600 dark:text-stone-300">{t('vocab.byArea', uiLang)}</h2><p className="mt-2 max-w-3xl text-xs leading-relaxed text-stone-500 dark:text-stone-400">{pick(lang, UI.uniqueWords)}</p><div className="mt-3"><MasteryLegend /></div>{first && renderGroup(first)}{rest.length > 0 && <details className="group mt-5"><summary className="flex min-h-11 cursor-pointer list-none items-center gap-1.5 text-sm font-medium text-stone-500 hover:text-amber-700 dark:text-stone-400 dark:hover:text-amber-400 sm:min-h-0"><span aria-hidden className="text-xs transition-transform group-open:rotate-90">▸</span>{t('vocab.moreBreakdowns', uiLang)}</summary>{rest.map(renderGroup)}</details>}</section>;
}
