...
interface Props { units: CourseUnit[]; spine: string[] }
...
export default function CurriculumPath({ units, spine }: Props) {
  ...
  return <div>
    ...
    {view === 'atlas' && <AtlasView topics={topics} selected={selected} goal={goal} routeIds={routeIds} completions={completions} level={level} strand={strand} lang={lang} onLevel={setLevel} onStrand={setStrand} onSelect={setSelectedId} onGoal={chooseGoal} />}
    {view === 'relations' && <RelationsView topics={topics} selected={selected} query={query} lang={lang} completions={completions} onQuery={setQuery} onSelect={setSelectedId} onGoal={chooseGoal} goal={goal} />}
  </div>;
}
...
interface AtlasViewProps {
  topics: CourseTopic[]; selected: CourseTopic; goal?: ActiveGoal; routeIds: Set<string>;
...
function AtlasView({ topics, selected, goal, routeIds, completions, level, strand, lang, onLevel, onStrand, onSelect, onGoal }: AtlasViewProps) {
  const visible = (topic: CourseTopic) => (level === 'all' || topic.level === level) && (strand === 'all' || topic.strand === strand) || routeIds.has(topic.id);
  return <div className="mt-6">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_19rem]">
      <div className="overflow-x-auto rounded-lg border border-stone-200 bg-stone-50 p-4 dark:border-stone-700 dark:bg-stone-900/40">
        <div className="min-w-[680px] space-y-5">
          {STRANDS.map(([id, label]) => {
            const row = topics.filter((topic) => topic.strand === id && visible(topic));
            if (!row.length) return null;
            return <section key={id} className="grid grid-cols-[7rem_1fr] gap-3"><h3 className="pt-3 text-xs font-semibold uppercase tracking-wide text-stone-400">{label}</h3><div className="flex flex-wrap gap-3">{row.map((topic) => <MapNode key={topic.id} topic={topic} selected={topic.id === selected.id} goal={topic.id === goal?.topicId} route={routeIds.has(topic.id)} completion={completions.get(topic.id)} onSelect={onSelect} />)}</div></section>;
          })}
        </div>
      </div>
      <TopicDetail topic={selected} topics={topics} completion={completions.get(selected.id)} lang={lang} isGoal={goal?.topicId === selected.id} onGoal={onGoal} />
    </div>
  </div>;
}
...
interface TopicDetailProps { topic: CourseTopic; topics: CourseTopic[]; completion?: Completion; lang: string; isGoal: boolean; onGoal: (id?: string) => Promise<void> }
function TopicDetail({ topic, topics, completion, lang, isGoal, onGoal }: TopicDetailProps) {
  ...
  return <aside className="rounded-lg border border-stone-200 bg-white p-5 dark:border-stone-700 dark:bg-stone-800">
    ...
    <RelationSummary label="Voraussetzungen" topics={prereqs} /><RelationSummary label="Baut darauf auf" topics={unlocks} />
...
interface RelationsViewProps {
  topics: CourseTopic[]; selected: CourseTopic; query: string; lang: string;
...
function RelationsView({ topics, selected, query, lang, completions, onQuery, onSelect, onGoal, goal }: RelationsViewProps) {
  ...
  return <div className="mt-6 grid gap-6 md:grid-cols-[17rem_1fr]">
    ...
    </aside><TopicDetail topic={selected} topics={topics} completion={completions.get(selected.id)} lang={lang} isGoal={goal?.topicId === selected.id} onGoal={onGoal} />
  </div>;
}
...
function RelationSummary({ label, topics }: { label: string; topics: CourseTopic[] }) { return <div className="mt-4 text-sm"><h3 className="text-xs font-semibold uppercase tracking-wide text-stone-400">{label}</h3><p className="mt-1 text-stone-600 dark:text-stone-300">{topics.length ? topics.map((t) => t.title_de).join(' · ') : 'Keine'}</p></div>; }
