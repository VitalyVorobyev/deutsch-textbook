...
**Current open-production contract:** every item has a positive `revision` (default 1), copied to new attempts. Writing and speaking require honest `met`/`needs work` assessment before and after revision/retry; the structured payload remains unverified practice and never affects mastery. Optional-artifact feedback is profile-scoped, merges latest-by-timestamp and is excluded from learning evidence.
...
- **`accept` on a vocab entry** — other correct typed answers for the EN/RU→DE production card. It exists because `de` is *three* things at once: the Wortliste key (matched against the manifest character for character), the answer shown on the back, and the answer the learner must type. For most words those coincide; for four classes they do not, and without `accept` the card marks correct German **wrong**:
  ...
  - a **reflexive verb's form fields** must also carry `sich` (`bewirbt sich` · `hat sich beworben`), because `deDetail` is built from `praesens_3sg` + `partizip2` — bare forms there teach a different verb (`unterhalten` = to entertain).
