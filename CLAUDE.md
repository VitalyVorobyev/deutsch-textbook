...
93/93 · 300/300 · 164/164, **Themen 70/70** and **Sprachhandlungen 38/41**, all since 2026-08-15.
The three that remain are interkulturelle Begegnung, Wissensdivergenz and Gefühle ausdrücken —
none of them a discourse function, all of them still real. The learner (Vitaly)
...
| using **Redaktion** or **Tonwerk** | [`docs/apps/redaktion.md`](docs/apps/redaktion.md) · [`docs/apps/tonwerk.md`](docs/apps/tonwerk.md) |
...
- `bun scripts/handlungen.ts <A1|A2|B1> [--unclaimed-only] [--beyond]` — **the same question asked of the can-dos**: do the 221 `outcomes` contain every communicative function the published standard expects? (102 of them cite one.) `data/handlungslisten/` is to outcomes what `data/strukturenlisten/` is to grammar rows, and it exists because a course can teach every structure the exam tests and never ask the learner to refuse an offer. **38/41 at A2 and B1 since 2026-08-15**: `gespraech-steuern` claims ten functions at once, which is what P26-10 predicted a single discourse-strategy topic would do. The three left — interkulturelle Begegnung, Wissensdivergenz, Gefühle ausdrücken — are not turn-management and want a different topic. `beyond` is expected here and is not a gap (a grammar outcome realises no language function). → [`data/handlungslisten/README.md`](data/handlungslisten/README.md)
...
- `bun run tonwerk` — **Tonwerk**, the audio studio's workbench (`apps/tonwerk`, React/Vite at :4340). It is the **only** interface to the listening studio and opens no files: everything it renders comes from the local Python engine (`atlas-listening serve`, `127.0.0.1:8765`), reached over HTTP with a bearer token that is minted per run and never written to disk. Its six sections are Übersicht · Szenen · Lesetexte · Prüfung · Klangbibliothek · Figuren. The one rule the app is built around: **QA is automatic, approval is not** — a signature names the sha256 of the master the reviewer played, and a re-render invalidates it. Publishing is the engine's (`scene publish-approved --yes`), never a button beside the render button. Guide: [`docs/apps/tonwerk.md`](docs/apps/tonwerk.md). Engine: [`tools/listening-studio/README.md`](tools/listening-studio/README.md).
...
- **A progress write is journaled before it is attempted, retried on a timer, and loud when late; a stalled or implausibly empty read renders an error, never a fresh-profile view, and never feeds `planReview`.** Every write behind the journal or `withPersistenceRetry` must be idempotent. No snapshot writer ever shrinks an existing file. **A write that settles AFTER its deadline still confirms itself** — `onLateSettle` clears the journal entry, because idb-keyval resolves on `transaction.oncomplete`, so a resolve is a commit. A journal entry that outlives its own landed write is the alert lying about the learner's data, and that is what a banner frozen at a count that never falls looks like. A late *rejection* is not a settle. → [ADR 0016](docs/adrs/0016-durable-progress-writes.md), amended by [ADR 0019](docs/adrs/0019-writes-single-flight-and-confirm-late.md)
- **One readwrite transaction on `progress` at a time; a burst of writes is ONE commit, not N.** All six keys share a single object store, so IndexedDB serialized them anyway — `enqueueUpdate` adds coalescing and bounds the wait. The deadline clock starts at **enqueue**, never at dequeue: queue time is silence, and silence is what the deadline converts into a fact. A timed retry drops the memoized handle and reconnects, which rescues a hung `indexedDB.open()` and **nothing else** — it cannot overtake a wedged transaction, and it leaks the old connection. The runner stops *waiting* for a batch at the write deadline (it never cancels), so one wedge cannot halt every later write. An op that can never be applied is quarantined to `da:journal-poison:<profileId>` rather than blocking the replay behind it, and a journal write localStorage refuses is named rather than swallowed. → [ADR 0019](docs/adrs/0019-writes-single-flight-and-confirm-late.md)
- **Reads get the same timed retries writes do, and every load surface distinguishes still-reading from read-failed from confirmed-empty.** All three through `withReadRetry(op, { surface })` (0/2/5 s, `StoreStallError` at 10 s, the stall logged to `da:stall-log:<profileId>` as counters only); a read that *rejects* is not retried, only silence is. A surface claiming something about progress renders `ProgressLoadError` with a retry, a badge renders an explicit unknown — **never a confident `0`**. Themen had neither and said “Загружаем ваш прогресс…” forever while 824 cards sat in the store. Two supporting rules: **nothing may sit between `getStore()` and its handle** (the A1 card-id migration was awaited there, hanging every page from behind a 784 KB read; it now has one owner — `PersistenceAlert`, once per launch — behind `da:cardid-migrated:<id>`, set only on success), and **a repair that rewrites a whole blob commits in one `update()`**, never a `get` then a `set`. → [ADR 0018](docs/adrs/0018-progress-reads-recover.md)
...
- **A gloss is a concise idiomatic translation (1–4 words) — the retrieval cue, never a definition.** A disambiguating parenthetical only where two shipped cards would share a production prompt; teaching goes in `note`. Measured by `bun scripts/gloss-shape.ts`, ratcheted by `tests/gloss-shape.test.ts` — lower a deck's number in the same commit that repairs it.
