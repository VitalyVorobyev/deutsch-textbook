...
- **A progress write is journaled before it is attempted, retried on a timer, and loud when late; a stalled or implausibly empty read renders an error, never a fresh-profile view, and never feeds `planReview`.** Every write behind the journal or `withPersistenceRetry` must be idempotent. No snapshot writer ever shrinks an existing file. → [ADR 0016](docs/adrs/0016-durable-progress-writes.md)
