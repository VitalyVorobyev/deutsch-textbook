...
- Profiles & progress store: `src/lib/profile.ts` (local learner profiles — no accounts/server; each profile is its own IndexedDB database; switcher in the header). There is **no hardcoded default profile**: on first open `resolveProfileState()` either adopts legacy pre-profile data (a populated `deutsch-atlas` DB becomes the `vitaly` profile) or the blocking `src/components/FirstRunGate.tsx` asks for a name; the last remaining profile cannot be deleted. `src/lib/store.ts` (profile-aware `getStore`; keys `attempts`/`cards`/`sessions`/`topics`; `mergeSnapshot`/`replaceSnapshot`) awaits that decision before touching IndexedDB and parks writes while the gate is up.
