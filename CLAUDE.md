...
- **`key_tokens` on a `translate` item — the tokens whose exact form the item's `focus` grades.** `src/lib/production.ts` is the one rule for scoring a typed sentence, and it exists because a translate item asks for a whole sentence: one mistyped character used to sink it *and* be recorded as a failure of the grammar it drilled. Two rules:
  ...
  Word order needs no declaration (tokens are compared positionally), so an item grading only placement (`modal-satzklammer`, `verbzweit`) needs `key_tokens` only for the verb it pins. Declare the tokens the tag is about: the auxiliary **and** participle for `haben-sein` (a Perfekt item grades both halves of the tense), the case-marked article for `dativ-artikel`, `gern` for `gern-moegen`. The validator rejects a `key_tokens` entry that does not occur in `answer`.

  **A `key_token` is matched by string, not by position** (`graded` in `src/lib/production.ts`), so if the word occurs **twice** in the answer, the tag grades *both* occurrences. `Tom hängt die Uhr über die Tür.` with `key_tokens: [die]` grades the accusative *object* article as well as the directional one — and a learner who writes `der Uhr` is then logged as a `wo-wohin` failure, which is not the confusion they had. Write the sentence so the graded token appears once (`Tom hängt den Kalender über die Tür.`), or pin a longer token that is unique. A false entry in the weakness signal is worse than a missing one: it sends training and drill authoring after a confusion the learner does not have.
