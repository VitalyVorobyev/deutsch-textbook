...
4. Probe family in `content/exercises/<level>/probe-<id>.yaml` — `role: probe`, 3 *parallel variants* (different tasks, same competence) over the topic's outcomes. Never listed in `exercises`. A probe that repeats a practice item measures recognition of that item, not retention.
...
7. Add the node (with 2–4 can-do `outcomes`) and its unit slot to `content/atlas.yaml`. **Every outcome must be referenced by at least one non-pretest exercise item or reading question** — an outcome nothing measures can never light up, and a probe on it can never arm.
