...
- **An item may never grade which word the author had in mind, and it must stay determinate when served alone** — mixed training strips the topic context that made it look determinate. A cloze gap asking for a lexical verb names the infinitive; a `translate` item pinning one of two interchangeable connectors (*denn*/*weil*) either names it in the `instruction` or accepts the sibling — **which one depends on whether the connector carries the `focus`**; a `table` answer key continues a `…` stub rather than repeating it, and the learner may do either — both are graded, because no authoring rule can reach an ambiguity that lives in the rendering.
- `mc` has exactly one correct answer. A `match` meaning-side right is a `{en, ru, uk?}` record, **never a mixed `"en / ru"` string**. `listen` text is ≤ ~10 words with numbers written as words. A committed `listening` item asks exactly one question, resolves a reviewed canonical artifact, and never treats playback as evidence. Readings gloss 6–10 phrases as `[[de::en::ru]]`.
