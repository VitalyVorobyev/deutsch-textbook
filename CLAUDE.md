...
- `bun run redaktion` — **Redaktion**, the editorial app (`apps/redaktion`, React on a Vite dev server at :4330): a **navigator over the language** with the course painted onto it, in **four maps** — the language, the course, one topic, the material — sharing one grammar of reading. **Sprachkarte** is the front door: ten grammatical strands × every CEFR level the schema knows as a density matrix, B2 a visibly empty column on purpose, with each level's instruments beneath it; **Einheiten & Themen** is the spine as cards toned against the level median, and **→ Thema** opens one topic with its prerequisites and dependants and *every* element it owns along the lesson cycle; **Bestand** indexes all fifteen `ElementKind`s behind one tab strip; **Strukturen**, **Fokus-Tags**, **Quellen** and the **Lücken** problem inbox give every point, confusion, published source and finding a routable page. Two rules carry the visual layer: **hue marks an exception or the reader's current position — quantity is drawn in ink** (a `bg-brand` fill for the ordinary state put ~90 amber squares on the front door and made the *normal* case the loudest thing on screen), and **no cell is unbounded**, in members (`Mehrere`) or in length (`Kurz`), because one long cell sets the height of its whole row. Every figure comes from `@da/content`, never recomputed here — a number that disagrees with `bun scripts/<name>.ts` means the app is wrong. The corpus is re-read when a file under `content/` or `data/` changes, so it is never stale. It runs under Bun (`bunx --bun vite`) because Vite's config loader uses Node's strict ESM resolution, which rejects the packages' extensionless imports. **It can also write two fields** — an exercise set's `stage:` and a topic's `status:` — through `PUT /__write`, and every rule about what that may touch lives in `@da/content/write`, never in the UI: it is an allowlist of (file class, field) pairs, it **splices into the source text instead of reserialising** (only 247 of 385 sets and manifests survive a `YAML.parseDocument`→`toString` round trip byte-for-byte, 18 under the library defaults), it refuses any write whose result would change a second key, and it runs `bun run validate` afterwards and **puts the original bytes back** if the corpus goes red. `apply: 'serve'` is what makes the endpoint unreachable from a build — not a flag; the plugin is simply not there, and the app degrades to the read-only report.
