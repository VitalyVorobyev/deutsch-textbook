...
describe('safe editorial article preview', () => {
  ...
  test('reports the current corpus language contract without treating German examples as translations', () => {
    ...
    expect(totals.en).toEqual({ complete: 51, partial: 0, unsupported: 0 });
    expect(totals.ru).toEqual({ complete: 51, partial: 0, unsupported: 0 });
    expect(totals.uk).toEqual({ complete: 51, partial: 0, unsupported: 0 });
    expect(totals.de).toEqual({ complete: 14, partial: 0, unsupported: 37 });
  });
});
