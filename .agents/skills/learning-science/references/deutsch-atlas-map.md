...
| **Multimedia / dual coding** (§10) | Grammar tables; semantic figures in `src/components/visuals/`; generated bases in `src/assets/illustrations/`; authentic stimuli in `content/documents/` | Use a figure only for a named spatial, transformational, temporal or sentence-topology job. Exact language stays in HTML/SVG, every relationship has a multilingual equivalent, and narrow layouts stack rather than scroll. Viewing remains input, never evidence. **Audio is not filed here** — see the next row; TTS reading visible text is the redundancy anti-pattern. |
