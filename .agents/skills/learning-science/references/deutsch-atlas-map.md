...
| **Generation / pretest** (§4) | Topic `pretest` field → `content/exercises/<level>/<id>-pretest.yaml`; "Was weißt du schon?" callout | 3 items before the article, the first a `cloze` (a generation attempt, not a recognition guess — P12-3); excluded from training, from mastery accuracy and from weakness evidence (guesses by design). |
...
| **Multimedia / dual coding** (§10) | Grammar tables; semantic figures in `src/components/visuals/`; generated bases in `src/assets/illustrations/`; authentic stimuli in `content/documents/`; provenance records in `data/asset-provenance.yaml` | Use a figure only for a named spatial, transformational, temporal or sentence-topology job. Exact language stays in HTML/SVG, every relationship has a multilingual equivalent, and narrow layouts stack rather than scroll. Apply the `authorship-provenance` skill to generated/simulated assets; `simulated` is provenance, not copyrightability. Viewing remains input, never evidence. **Audio is not filed here** — see the next row; TTS reading visible text is the redundancy anti-pattern. |
...
| **Comprehensible input — intensive & extensive** (§11) | Graded readings `content/reading/**` → `ReadingText.tsx`; `[[phrase::en::ru]]` glosses; each reading's `kind` (`intensive` or `extensive`); CEFR discipline | An **intensive** reading is ~90–130 words, 6–10 glosses, 2–4 comprehension questions. An **extensive** reader is 250–400 words, at most 2 gist questions and roughly one gloss per 40 words — bounds the validator enforces, because a long text quizzed line by line is just a long intensive text. Seven connected *Lena in Bremen* episodes now form a small A1–A2 strand; the gap is breadth and volume, not existence. Never call an intensive reading extensive. |
| **Pushed output / productive recall** (§12) | EN/RU→DE typed flashcards; `translate`; `listen`; open `write`/`speak` (minimal ceremony: attempt → model → done, requirements as guidance); the **item-mix bar** in `scripts/validate.ts` | `write` and `speak` log practice evidence, not verified accuracy. Audit measured and unverified output separately, using the actual `responseMode`. The bar is enforced per topic over the union of its `role: practice` sets — at least 2 `translate` items, `mc` at most a third, `mc`+`match`+`order` at most 45% — so a new unit cannot drift back into a recognition quiz. |
...
| **Text design / prose shape** (§17) | `content/topics/**/*.mdx` article bodies; the cap in `src/lib/prose-shape.ts`; reporter `scripts/prose-shape.ts`; rules in `docs/authoring/article-prose.md` | `## Erklärung` splits into `### German subsections` (outside `<Bilingual>`, so they survive every language mode), each keeping its table beside its prose; one claim per paragraph, hard-capped at 120 words per half by `bun run validate` and by `tests/prose-shape.test.ts`. The convention is now universal at A2 and B1; it was not always — nine A2 topics had it while all five B1 topics then written lacked it, which is how one paragraph reached 397 words against an A1/A2 maximum of 107. What no count can see stays with the author: whether a heading names its confusion, and whether a stated fact is ever drilled. |
...
Run `bun run progress:audit --profile <slug>` (historical folder:
`progress/vitaly/`) — never Read a raw snapshot; they run 300 KB+ and the audit
aggregates everything — and report **numbers**, not impressions:
...
progress":
1. Run `bun run progress:audit --profile <slug>` (never Read the raw snapshot);
   it aggregates **parts-weighted** error rates per focus tag (same logic as
   `weakFocuses` in `src/lib/weakness.ts`: last ~30 attempts, weak = ≥4 attempts
   and ≥35% errors) and high-`lapses` cards. Triage its grading-review queue
   *before* reading the weak-focus table — withheld attempts distort it.
