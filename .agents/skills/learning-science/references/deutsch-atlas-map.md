...
| **Comprehensible input — intensive & extensive** (§11) | Graded readings `content/reading/**` → `ReadingText.tsx`; `[[phrase::en::ru]]` glosses; each reading's `kind` (`intensive` or `extensive`); CEFR discipline | An **intensive** reading is ~90–130 words, 6–10 glosses, 2–4 comprehension questions. An **extensive** reader is 250–400 words, at most 2 gist questions and roughly one gloss per 40 words — bounds the validator enforces, because a long text quizzed line by line is just a long intensive text. Five connected *Lena in Bremen* episodes now form a small A1–A2 strand; the gap is breadth and volume, not existence. Never call an intensive reading extensive. |
