...
| **Pushed output / productive recall** (§12) | EN/RU→DE typed flashcards; `translate`; `listen`; revision-based `write`; local record/replay `speak` | `write` and `speak` log practice evidence, not verified accuracy. Audit measured and unverified output separately, using the actual `responseMode`. |
