...
| **Text design / prose shape** (§17) | `content/topics/**/*.mdx` article bodies; the cap in `src/lib/prose-shape.ts`; reporter `scripts/prose-shape.ts`; rules in `docs/article-prose.md` | `## Erklärung` splits into `### German subsections` (outside `<Bilingual>`, so they survive every language mode), each keeping its table beside its prose; one claim per paragraph, hard-capped at 120 words per half by `bun run validate` and by `tests/prose-shape.test.ts`. Nine A2 topics had the convention and all five B1 topics lacked it, which is how one paragraph reached 397 words against an A1/A2 maximum of 107. What no count can see stays with the author: whether a heading names its confusion, and whether a stated fact is ever drilled. |
