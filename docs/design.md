...
The site is a static build hosted on Cloudflare Pages at `atlas.vitavision.dev`, deployed by
`.github/workflows/pages.yml` and served at the root — `withBase` (`src/lib/url.ts`) remains the
one helper, so a subpath mirror and the Tauri shell stay possible. The same build is an
