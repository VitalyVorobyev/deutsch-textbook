...
- **Determinacy, across every type: an item may never grade which word the author had in mind.** The four faces it has worn so far, each found by a learner producing correct German and being told it was wrong:
  ...
  - a **`table`** row whose `given` stub ends in an ellipsis, where the learner cannot see whether the word the stub just handed over belongs in what they type. **A trailing `…` in a `given` cell declares a continuation prompt**, and the answer key continues it: `Ich komme nicht, weil …` + `weil ich heute arbeite` reads back as *weil weil*, and the key is the model answer shown after a miss. Validator-enforced, keyed on the ellipsis precisely so a paradigm table — where Nominativ `die` and Akkusativ `die` are both genuinely `die` — is untouched; write the pattern without an ellipsis if you mean a morphological placeholder (`Superlativ: am` beside `besten`).

    **The learner is not held to that choice, and this is the one case in this list where the fix was not an authoring rule.** `TableFill` draws cells as independent `<td>`s, so nothing on screen joins the stub to the input — an authoring contract cannot reach the ambiguity, and picking a side just moves it. `a2/verbindungen-folgen:table-drei-wortarten` graded the connector *in* at revision 1 and *out* at revision 2 and returned a false 0/3 both times, from a learner whose word order was right in all three rows on both occasions. So `acceptedCellAnswers` (`src/lib/table.ts`) accepts the answer with or without the stub's trailing word, derived from the stub rather than authored — nothing to add per item, and `validate.ts` imports the same function so the two cannot disagree about what was handed over.

  - a **`translate`** item that is determinate in one prompt language and not in another. `prompt_en` and `prompt_ru` are the same sentence written independently (below), and independence means they do not carry the same information: **Russian and Ukrainian mark no definiteness on a bare noun**, so «Завтра я куплю лампу» leaves both *eine Lampe* and *die Lampe* open where *Tomorrow I am buying a lamp* does not. `a1/akkusativ:uebersetzen-lampe` rejected `Morgen kaufe ich die Lampe` from a learner reading Russian, and the grading-review ruling that upheld it quoted the English prompt — because `scripts/progress-audit.ts` printed only that half until 2026-08-03. Resolve it the same way as the connector case, by the same test: definiteness is carrier material under `focus: verbzweit`, so the sibling determiner joins `accept` (closed over the word-order alternates the item already has). Where the underdetermined element *is* the focus, the German has to be reachable from the prompt the learner sees — fix the prompt, not the accept list. **Not validator-enforced**, deliberately: whether a bare Russian noun leaves a German determiner free is a semantic judgement with no closed group to check against, unlike `INTERCHANGEABLE_CONNECTORS`. The check that exists is authorial — read `prompt_ru` and `prompt_uk` and ask what each of them fixes.

  **This class hides behind curriculum order — and behind the reviewer's own language.** `a2/arbeit-beruf` teaches *denn* and its article says *weil* "comes later", so on the topic's own page the task really is determinate. Mixed training then serves the item stripped of that context, months after *weil* was taught. **An item is determinate only if it is determinate when served alone, in every prompt language it ships** — that is the test to apply, because interleaving guarantees the first and the learner's Lernsprache guarantees the second.
...
- **`key_tokens` on a `translate` item — the tokens whose exact form the item's `focus` grades.** `packages/grading/src/production.ts` is the one rule for scoring a typed sentence, and it exists because a translate item asks for a whole sentence: one mistyped character used to sink it *and* be recorded as a failure of the grammar it drilled. Two rules:
  ...
  **A `key_token` is matched by string, not by position** (`graded` in `packages/grading/src/production.ts`), so if the word occurs **twice in the rendering that defines it**, the tag grades *both* occurrences. `Tom hängt die Uhr über die Tür.` with `key_tokens: [die]` grades the accusative *object* article as well as the directional one — and a learner who writes `der Uhr` is then logged as a `wo-wohin` failure, which is not the confusion they had. **`bun run validate` rejects this** in `answer`, and likewise when an alternative-only token repeats in the accepted rendering that introduces it. Rewrite the sentence so the graded token appears once (`Tom hängt den Kalender über die Tür.`), or pin a different word that is unique — tokens are whole words, so a multi-word phrase cannot be pinned. Where the repetition is genuinely the same decision twice, it is still worth splitting: the item would otherwise weigh one confusion twice as heavily as its siblings. A false entry in the weakness signal is worse than a missing one: it sends training and drill authoring after a confusion the learner does not have.

  **The match also folds case at sentence heads**, because there capitalization is orthography, not grammar: a pinned lowercase token is graded when it stands capitalized at the head of *any* sentence of a rendering (not just the first — a two-sentence `accept` has two heads), and a pin capitalized at a sentence head derives a lowercase twin that matches **anywhere**. So the string your pin grades is bigger than the string you wrote: `Nach der Arbeit gehe ich nach Hause.` with `Nach` pinned grades the *nach* of *nach Hause* too. The validator asks the grader itself (`gradedTokenPositions`) and **fails any rendering whose graded positions outnumber the pins**. When the extra occurrence really is the same decision twice (an `accept` that repeats the pinned auxiliary after *und*), declare it by pinning the token once per intended slot — `key_tokens: [sind, sind, geflogen, geblieben]` — which changes nothing in grading and tells the guard both slots are meant. Mid-sentence capitalization is never folded: `Sie` and `sie` stay different words.
...
**Both ratios count written items only — `audio-comprehension` is excluded from the numerator *and* the denominator.** Every argument above is about production: recognition is capped because the learner "never has to produce it", and a listening comprehension task cannot ask for production without conflating two skills. Counting it in the denominator alone — which is what the rule did at first — let each recorded item buy its topic more room for written recognition, measurably loosening both caps on most of the corpus the moment the recordings attached. The same exclusion applies to the stricter placement caps below. Pinned by `tests/item-mix.test.ts`, whose second test fails if no practice set owns audio — without that, the first test proves nothing.
...
| Rule | Why |
| --- | --- |
| **Every topic of the level, ≥ 2 items** | One item is a coin flip, and the verdict is permanent. Sizing follows from this: A1 is 24 items over 10 topics, A2 is 52 over 25. |
