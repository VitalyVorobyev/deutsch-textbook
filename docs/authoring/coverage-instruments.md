...
**A level with no source reports `anchored: false`, never 100% of nothing.** B1 is that level today:
32/32 against no external list, which is exactly the state A1 was in while missing four structures.
`data/strukturenlisten/README.md` names the missing documents and what each costs.

Today: **A1 92/93 · A2 138/138** claimed, 43 rows `beyond` across the three levels.
...
Today: median teaching items per confusion **A1 12 · A2 8 · B1 4**, production **6 · 6 · 3**, and
10 of 35 B1 tags in exactly one practice file against 4 of 27 at A1.
