...
A2 unit quality gate in [the A1 learning audit](../quality/a1-learning-audit.md). This document decides the
...
(`taughtSurface` in `packages/content/src/coverage.ts`). A `~` you cannot pay for with an article table, a reading
...
The taxonomy is an allowlist. A tag must appear in the [focus-tag table](../authoring/focus-tags.md) **and** in
`focusIntroducedBy` (`packages/content/src/focus-tags.ts`), naming the topic that introduces it, or validation
...
ankommen are already cards), `perfekt-a1-verben`.
...
`perfekt-a1-verben`, `schule-arbeit`, `verben-grundwortschatz` (heiraten).
...
`docs/authoring/focus-tags.md`, ratchet in `tests/grammar-coverage.test.ts` raised in the same commit).
...
2. Author one complete unit per bundle: atlas node and unit slot, article, three-item pretest,
   ...
   [`focus-tags.md`](../authoring/focus-tags.md) and `focusIntroducedBy` (`packages/content/src/focus-tags.ts`). A genre unit
...
- Every A2 unit clears the twelve-point A2 unit quality gate in [the audit](../quality/a1-learning-audit.md).
...
`docs/authoring/authoring-checklists.md:18`.
...
denominator up, percentage down. `CLAUDE.md`, `docs/authoring/coverage-instruments.md` and the ratchet in
...
**Correction 2026-08-05, measured at wave 3's start: waves 3–4 are smaller than the table says.**
The wave-3/4 sizes above were drawn against the 2026-08-03 missing list, but a shipped entry can
cover more Wortliste rows than its one headword — `accept` variants and sibling rows (*Lastwagen*
also pays *Laster*, *Frucht* pays *Früchte*) — and the wave-1/2 decks absorbed some neighbouring
wave-3/4 words outright. After waves 1–2 the actual remainder is **706 missing = 73 no-card rows
(every row of the table below, recounted against the ✗ list) + 633 card words**
(`bun scripts/coverage.ts B1`), against the nominal 790. The deck names and themes stand; sizes
land where the remaining words are, below the 30–45 band where the field is already drained. Wave
3a shipped as `text-schreiben-b1` 19 · `sprechen-lernen-b1` 23 · `computer-internet-b1` 29 ·
`medien-digital-b1` 25 · `abstrakte-begriffe-b1` 37 · `wirtschaft-handel-b1` 19 — 152 words, all
`rec`. Wave 3b closed the wave as `recht-kriminalitaet-b1` 38 · `politik-staat-b1` 36 ·
`gesellschaft-migration-b1` 25 · `bildung-schulsystem-b1` 28 · `wissenschaft-technik-b1` 20 —
147 words, all `rec`; wave 3 in total 299 against the nominal 407.
...
### Historical 73-row no-card proposal — superseded 2026-08-15

The table below records the planning state from 2026-08-06 and is retained as audit history, not as
the current contract. The P27-3f row-by-row review rejected its blanket `~` conclusion: 12
written/official forms now have recognition cards, six frequent independent forms have productive
cards, and only 55 genuine bound pieces/frames are grammar or word formation. The normative
disposition of every row is in the
[A1–B1 completeness audit](a1-b1-completeness-audit.md#b1-wortliste-tail-7373-classified-and-delivered).
B1 now measures 3416/3416 (3297 cards, 119 grammar, 0 missing, 0 late).

Historically, every row was proposed as a new `~` in `data/goethe-b1-wortliste.txt`, and `bun run validate` **hard-fails a `~`
whose word the taught surface does not contain**. Measured against today's surface
(`addresses(taughtSurface(), word)`, `packages/content/src/coverage.ts`): **29 of the 73 already pass, 44 do
