...
- Internal grammar moved from 24/28 to 28/28; external A1 remained 93/93 and lexical A1 remained
  673/673. The exact DTZ tail moved from 22 to 21 through `nachsilbe-chen`, not an alias.
- `wortbildung` was added as a real topic with article, pretest, three functional practice stages,
  reading, placement evidence and delayed probes. The `Kuchen` boundary is taught explicitly so
  final letters are not mistaken for a productive diminutive.
- `praesens-wortstellung` and `artikel-genus` gained substantive coordination and demonstrative
  sections, not just inventory tags. New contexts span work, weather, transport, appointments and
  objects outside the owner lesson.
- Four previously taught A1 confusions without delayed checks (`akkusativ-pronomen`,
  `haben-wendungen`, `gern-moegen`, `von-bis-in-zeit`) gained three fresh probe variants each.
- The five remaining A1 articles without addressable explanation sections were restructured by
  actual decisions. The review corrected the false claim that *Ich bin hungrig* is merely bookish:
  it is grammatical; the error is mixing the noun *Hunger* with *sein*.
